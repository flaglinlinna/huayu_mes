...
//            in.close();
    ...
    public static void exportBySheetAndRow(HttpServletResponse response, List<List<Map<String, Object>>> importlist,XSSFWorkbook workbook,Integer sheetNum, String[] mapNames,String filePath, String file_name,Integer titleRow) throws Exception {
        //获取数据集
        //获取第一页
        Resource resource = new ClassPathResource(filePath);
        InputStream in = resource.getInputStream();
...
//        XSSFSheet sheet = workbook.getSheetAt(0);
        //循环字段名数组，创建标题行-16
        Row row = sheet.getRow(0);
        List<Map<String, Object>> datalist = importlist.get(k);
        //创建普通行
        for (int i = 0; i < datalist.size(); i++) {
            //因为前两行已经用于创建标题行，故从第二行开始创建
            row = sheet.createRow(i + titleRow);
