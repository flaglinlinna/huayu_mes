...
//    @Query(value = )
    ...
    @Query(value = "select r.id from sys_role r left join sys_user_role m on r.id=m.role_id where r.del_flag=0 and r.status=0 and m.del_flag=0 and  m.user_id=?1  ", nativeQuery = true)
    public List<Map<String, Object>> getRoleIdByUserId(Long userId);
