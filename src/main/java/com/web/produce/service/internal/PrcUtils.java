...
public class PrcUtils {
    ...
    //返回扫描界面显示的数据 prc_mes_barcode_info_get
    public List getBarcodeByPrc(String company,String facoty,String user_id,int type, String barcode) throws Exception {
        List resultList = (List) jdbcTemplate.execute(new CallableStatementCreator() {
            @Override
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                String storedProc = "{call  prc_mes_barcode_info_get (?,?,?,?,?,?,?,?)}";// 调用的sql
                CallableStatement cs = con.prepareCall(storedProc);
                cs.setString(1, company);
                cs.setString(2, facoty);
                cs.setString(3, user_id);
    ...
    public List getBarHistoryPrc(String company,String facoty,String user_id,Integer errFlag,
        ...
        List resultList = (List) jdbcTemplate.execute(new CallableStatementCreator() {
            ...
            public CallableStatement createCallableStatement(Connection con) throws SQLException {
                ...
                cs.setString(1, company);
                cs.setString(2, facoty);
                cs.setString(3, scanFrom);
                cs.setString(4, ScanType);
                cs.setInt(5, errFlag);
