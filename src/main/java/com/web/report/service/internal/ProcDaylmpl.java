 ...
 * 过程检验日报表
 */
@Service(value = "ProcDayService")
@Transactional(propagation = Propagation.REQUIRED)
public class ProcDaylmpl extends ReportPrcUtils implements ProcDayService {
	...
	public List getReportPrc(String facoty,String company,String user_id,String beginTime,
		...
		List resultList = (List) jdbcTemplate.execute(new CallableStatementCreator() {
			...
			public CallableStatement createCallableStatement(Connection con) throws SQLException {
				String storedProc = "{call  prc_mes_rpt_chk_process(?,?,?,?,?,?,?,?,?,?,?,?,?)}";// 调用的sql
	...
	public List getDetailReportPrc(String facoty,String company,String user_id,String dep_id, 
		...
		List resultList = (List) jdbcTemplate.execute(new CallableStatementCreator() {
			@Override
			public CallableStatement createCallableStatement(Connection con) throws SQLException {
				String storedProc = "{call  prc_mes_rpt_chk_process_det(?,?,?,?,?,?,?,?,?,?,?,?,?)}";// 调用的sql
				CallableStatement cs = con.prepareCall(storedProc);
				cs.setString(1, facoty);
				cs.setString(2, company);
				cs.setString(3, user_id);
