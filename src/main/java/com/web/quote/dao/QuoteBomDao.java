...
public interface QuoteBomDao extends CrudRepository<QuoteBom, Long>,JpaSpecificationExecutor<QuoteBom>{
	...
	@Query(value = " SELECT DISTINCT p.BS_NAME as BSCOMPONENT,min(p.BS_ORDER) as BS_ORDER from PRICE_QUOTE_PROCESS p where p.ITEM_TYPE not like '辅料%' and p.PK_QUOTE = ?1 and p.bs_Element =  ?2 GROUP BY p.BS_NAME order by BS_ORDER ", nativeQuery = true)
