...
function dealData(){
	...
	for(var i=0;i<kanbanData.length;i++){
		...
		series1.push(kanbanData[i].QTY_PLAN);//标准工时
		series2.push(kanbanData[i].QTY_DONE);//实际工时
		series3.push(kanbanData[i].RATE_DONE);//生产效率
	}
	...
	$("#showLine").text("开线数："+kanbanDataList.data.LINE_NUM_NOW+"     "+"总线体数："+kanbanDataList.data.LINE_NUM_PLN);
}
		    ...
		    series: [
		        {
		            ...
		            data: [
		                {value: emp_off, name: '缺勤人数',selected: true},
...
function setTable(kanbanData){
	...
	for(var j = 0; j < kanbanData.length; j++) {
		 ...
		 html += '<tr><td>'+arr.LINER_NAME+'</td><td>'+arr.NUM_EMP_ON+'</td><td>'+arr.NUM_EMP_PL
		 +'</td><td>'+arr.QTY_DONE+'</td><td>'+arr.QTY_PLAN+'</td><td>'+arr.RATE_DONE+'%</td></tr> ';
    }
	 $("#tableList").empty();
