...
$(function() {
	layui.use([ 'form', 'table','tableSelect' ], function() {
		...
		tableIns = table.render({
			elem : '#colsList',
			url : context + '/basePrice/procRole/getList',
			method : 'get' // 默认：get请求
			,
			cellMinWidth : 80,
		...
		tableSelect = tableSelect.render({
				...
				table : {
					//width : 220,
					url : context
						+ '/basePrice/proc/getList',
					method : 'get',

					cols : [ [ {type : 'radio'},// 多选 radio
						{field : 'procName', title : '工艺名称', width : 200},
						{field : 'workCenter', title : '工作中心'}
					] ],
					page : true,
					request : {
						pageName : 'page' // 页码的参数名称，默认：page
...
function getRoleListByType(id) {
	CoreUtil.sendAjax("/basePrice/procRole/getListByWgId?wgId="+id, "",
		function(data) {
			if (data.result) {
				tableIns1.reload({
					done : function(res, curr, count) {
...
function addSubmit(pkItemTypeWg,roleIds) {
	...
	CoreUtil.sendAjax("/basePrice/procRole/add", JSON.stringify(params), function(
		data) {
		if (data.result) {
...
function editSubmit(obj) {
	CoreUtil.sendAjax("/basePrice/procRole/edit", JSON.stringify(obj.field),
			function(data) {
				if (data.result) {
					layer.alert("操作成功", function() {
						layer.closeAll();
...
function delData(obj, id, name) {
	if (id != null) {
		...
		layer.confirm('您确定要删除该名称为:' + name + ' 的工艺关联角色信息吗？', {
			btn : [ '确认', '返回' ]
		// 按钮
		}, function() {
			CoreUtil.sendAjax("/basePrice/procRole/delete", JSON
